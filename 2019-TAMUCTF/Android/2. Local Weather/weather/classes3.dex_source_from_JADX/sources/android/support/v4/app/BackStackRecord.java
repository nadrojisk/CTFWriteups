...
final class BackStackRecord extends FragmentTransaction implements BackStackEntry, Runnable {
    ...
    private boolean configureTransitions(int containerId, TransitionState state, boolean isBack, SparseArray<Fragment> firstOutFragments, SparseArray<Fragment> lastInFragments) {
        ...
        ViewRetriever c01631 = new ViewRetriever() {
            public View getView() {
        ...
        if (transition != null) {
            FragmentTransitionCompat21.addTransitionTargets(enterTransition, sharedElementTransition, exitTransition, sceneRoot, c01631, state.nonExistentView, state.enteringEpicenterView, state.nameOverrides, enteringViews, exitingViews, namedViews, renamedViews, sharedElementTargets);
