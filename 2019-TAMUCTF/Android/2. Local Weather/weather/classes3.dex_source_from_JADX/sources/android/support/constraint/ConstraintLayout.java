...
public class ConstraintLayout extends ViewGroup {
                    ...
                    if (layoutParams.circleConstraint != -1) {
                        ...
                        if (layoutParams.baselineToBaseline != -1) {
                            ...
                            if (!(target == null || view == null || !(view.getLayoutParams() instanceof LayoutParams))) {
                                ...
                                widget.getAnchor(Type.BASELINE).connect(target.getAnchor(Type.BASELINE), 0, -1, Strength.STRONG, 0, USE_CONSTRAINTS_HELPER);
            ...
            if (child.getVisibility() != 8) {
                ...
                if (!(params.isGuideline || params.isHelper)) {
                    ...
                    if (width == 0 || height == 0) {
                        ...
                        boolean bothHorizontal = (widget.getAnchor(Type.LEFT).getTarget() == null || widget.getAnchor(Type.RIGHT).getTarget() == null) ? false : USE_CONSTRAINTS_HELPER;
                        ...
                        boolean bothVertical = (widget.getAnchor(Type.TOP).getTarget() == null || widget.getAnchor(Type.BOTTOM).getTarget() == null) ? false : USE_CONSTRAINTS_HELPER;
                        if (width != 0 || height != 0 || !bothHorizontal || !bothVertical) {
                            int childHeightMeasureSpec;
                            didWrapMeasureWidth = false;
                            didWrapMeasureHeight = false;
                            boolean resolveWidth = this.mLayoutWidget.getHorizontalDimensionBehaviour() != DimensionBehaviour.WRAP_CONTENT ? USE_CONSTRAINTS_HELPER : false;
                            boolean resolveHeight = this.mLayoutWidget.getVerticalDimensionBehaviour() != DimensionBehaviour.WRAP_CONTENT ? USE_CONSTRAINTS_HELPER : false;
                            if (!resolveWidth) {
                                widget.getResolutionWidth().invalidate();
                            }
                            if (!resolveHeight) {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == 1073741824 && heightMode == 1073741824 && widthSize == this.mLastMeasureWidth && heightSize == this.mLastMeasureHeight) {
        }
        boolean sameMode = (widthMode == this.mLastMeasureWidthMode && heightMode == this.mLastMeasureHeightMode) ? USE_CONSTRAINTS_HELPER : false;
        if (sameMode && widthSize == this.mLastMeasureWidthSize && heightSize == this.mLastMeasureHeightSize) {
        }
        if (sameMode && widthMode == Integer.MIN_VALUE && heightMode == 1073741824 && widthSize >= this.mLastMeasureWidth && heightSize == this.mLastMeasureHeight) {
        }
        if (sameMode && widthMode == 1073741824 && heightMode == Integer.MIN_VALUE && widthSize == this.mLastMeasureWidth && heightSize >= this.mLastMeasureHeight) {
        }
